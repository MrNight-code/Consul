...
Ubicación: `src/Consulcon.API/Controllers/Facturacion/PagoController.cs` (Existente, endpoint agregado)
...
- `GET /recibos`: Lista los recibos generados con filtros opcionales.
  - **Parámetros (Query)**:
...
- `POST {id}/generar-recibo`: Genera un recibo PDF para una transacción de pago existente.
...
- `GET /recibos/{filename}`: Descarga el archivo PDF generado.
