...
-- Origen: {{STAGING_DB}}
-- Destino: db_condominio_bosques_colina
...
SELECT pk_tipousuario, descripcion FROM {{STAGING_DB}}.tipousuario;
...
FROM {{STAGING_DB}}.banco;

-- Formas de Pago
INSERT INTO forma_pago (id_forma_pago, descripcion)
SELECT pk_formpago, descripcion FROM {{STAGING_DB}}.formpago;
...
SELECT pk_persona, IFNULL(nombre, 'Sin Nombre'), ci, fechanac, sexo, estadocivil, IF(activo='S', 1, 0)
FROM {{STAGING_DB}}.persona;

-- 2. Contactos desde la tabla 'persona' (Celular, Telefono, Email)
INSERT INTO medio_contacto (id_persona, tipo, valor, es_principal)
SELECT pk_persona, 'Celular', celular, 1 FROM {{STAGING_DB}}.persona WHERE celular IS NOT NULL AND celular != ''
UNION
SELECT pk_persona, 'Telefono', telefono, 0 FROM {{STAGING_DB}}.persona WHERE telefono IS NOT NULL AND telefono != ''
UNION
SELECT pk_persona, 'Email', email, 0 FROM {{STAGING_DB}}.persona WHERE email IS NOT NULL AND email != '';

-- 3. Contactos desde la tabla externa 'telefono'
INSERT INTO medio_contacto (id_persona, tipo, valor, es_principal)
SELECT fk_persona, IFNULL(tipo, 'Telefono'), numero, 0
FROM {{STAGING_DB}}.telefono 
WHERE fk_persona IS NOT NULL;

-- 4. Contactos desde la tabla externa 'correo'
INSERT INTO medio_contacto (id_persona, tipo, valor, es_principal)
SELECT fk_persona, IFNULL(tipo, 'Email'), email, 0
FROM {{STAGING_DB}}.correo 
WHERE fk_persona IS NOT NULL;
...
SELECT pk_contrato, fk_propiedad, 
       CASE WHEN CAST(fecha AS CHAR) LIKE '0000%' THEN NULL ELSE fecha END, 
       CASE WHEN CAST(fechaini AS CHAR) LIKE '0000%' THEN NULL ELSE fechaini END, 
       CASE WHEN CAST(fechafin AS CHAR) LIKE '0000%' THEN NULL ELSE fechafin END, 
       CASE WHEN CAST(fechaingreso AS CHAR) LIKE '0000%' THEN NULL ELSE fechaingreso END, 
       expensa, 
       CASE WHEN valido = 'SI' THEN 'Vigente' ELSE 'Finalizado' END, motivo, fk_usuarioinsert
FROM {{STAGING_DB}}.contrato;

INSERT INTO contrato_participante (id_contrato, id_persona, rol_contrato, fecha_alta, fecha_baja, activo)
SELECT fk_contrato, fk_persona, tipo, 
       CASE WHEN CAST(fecharegistro AS CHAR) LIKE '0000%' THEN NULL ELSE fecharegistro END, 
       CASE WHEN CAST(fecharetiro AS CHAR) LIKE '0000%' THEN NULL ELSE fecharetiro END, 
       IF(activo='S', 1, 0)
FROM {{STAGING_DB}}.persona_contrato;
...
FROM {{STAGING_DB}}.servicio_contrato;
...
SELECT 'MIGRACIÓN COMPLETA DESDE {{STAGING_DB}} A db_condominio_bosques_colina' AS Resultado;
